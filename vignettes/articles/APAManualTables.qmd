...
apa_flextable(d, 
              table_width = 1, 
              parameter_formatter = my_formatter) 
```
...
d <- d |> 
  ...
  mutate(`F` = align_chr(`F`, 
                                 accuracy = .01),
         `papa7starcolumn` = p2stars(p, superscript = T), 
         .after = `F`) |>
...
Suppose the default formatting is not quite right. The `apa_parameters` function makes use of the default output of `parameter_formatter` function to make its changes. 
...
apa_parameters(m2, parameter_formatter = my_formats) |> 
  apa_flextable()
```
...
d <- d_analysis |>
  ...
  mutate(
    papa7starcolumn = p2stars(pp, superscript = T),
...
d |> 
  ...
  width(width = 6.5 * c(3, .6,.4, 1,1, 1, .6,.4, 1,1) / 10)
...
tibble(
  ...
  add_break_columns(RMSEA_Value, .before = T) |> 
  ...
  apa_flextable(markdown_body = TRUE,
                layout = "fixed", 
                font_size = 11) |> 
  width(width = c(1.8, .45, .35, .45, .05, .4, .85,.35, .5, .5, .4, .4)) 
```
...
d <- fit |> 
  ...
  mutate(stars = p2stars(p, superscript = T),
...
tibble(
  ...
  mutate(
    Chi2 = paste0(align_chr(Chi2, accuracy = 0.001),"^\\*\\*\\*^")) |> 

  apa_flextable(
    parameter_formatter = parameter_formatter(accuracy = 0.001),
    layout = "fixed") |> 
...
tibble(
  ...
  width(width = c(2.5, 4)) |> 
  padding(j = 1, 
          i = ~!is.na(`Discourse and dimension`), 
          padding.left = 15)
```
