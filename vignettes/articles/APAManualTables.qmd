...
library(parameters)
library(performance)
set_flextable_defaults(theme_fun = theme_apa, font.family = "Times New Roman")
...
As an exercise, I decided to reproduce most of the sample tables in the APA Manual's Chapter 7, which covers the display of tables. There are a number of differences between the original tables and the recreations. I stick with Times New Roman as my font. I add little breaks between groupings in multilevel headers.

Things I usually change, but I do not mess with here:

* I like decimal alignment, and I do not like it when footnotes throw off the alignment of text. However, the sample tables are not too fussy on such matters.
* I like column headings to have the same alignment as the body cells under them. APA style usually has centered headings for everything.
...
ft_71 |> 
  style(i = ~ Wave == 2,
                   pr_t = officer::fp_text(color = "dodgerblue")) |>
...
footnote(
  i = ~ Wave == 1 &
    Grade == 3,
  j = c("Girls_With", "Girls_Without"),
  value = as_paragraph_md(
    c("^\u00A0^A *specific note* appears in a separate 
...
d_citizenship |>
  ...
  mk_par(
    i = ~ Wave == 1 & Grade == 3,
    j = "Girls_With", 
    value = as_paragraph(Girls_With, as_sup("a"))) |> 
  apa_style(table_width = .6) |> 
  mk_par(
    i = ~ Wave == 1 & Grade == 3,
    j = "Girls_Without", 
    value = as_paragraph(Girls_Without, as_sup("b"))) |> 
  append_chunks(
    i = ~ Wave == 2 & Grade == "Total",
    j = "Boys_With", 
    as_chunk("*")) |>
  apa_style(table_width = 1, line_spacing = 1.5)
```
...
tibble::tribble(
  ...
  mutate(`Cronbach’s α` = signs(
    `Cronbach’s α`, 
    trim_leading_zeros = TRUE)) |> 
    mutate(across(where(is.numeric), align_chr)) |> 
  mutate(Range = align_chr(Range, center = "–")) |> 
  flextable() |> 
  apa_style(layout = "fixed") |> 
  colformat_md() |> 
  italic(i = 1, j = 2:3, part = "header") |> 
  align(align = "center") |>
  align(j = 1) |> 
  padding(j = 1, 
  ...
  mutate(
    ...
    p = map_chr(data, \(d) {
      t.test(x ~ Group, data = d)$p.value |>
        apa7::apa_p(min_digits = 3)
    }),
  ) |>
  select(-data) |>
  unnest(descriptives) |>
  mutate(across(-Scale, \(x) align_chr(x, center = " "))) |> 
...
tibble::tribble(
  ...
  apa_flextable(table_width = 1) |> 
  align(j = 1, align = "left") |> 
  colformat_md(j = 7:9, part = "header") |> 
  colformat_md(j = 1) |> 
  italic(i = 2, part = "header")
```
...
Depersonalization   	3.00	Control	1.24	0.93" %>%
  ...
  apa_flextable(row_title_column = "v", 
                ...
                layout = "fixed", 
                table_width = 1) |>
  align(j = c(2:8, 11:12), align = "center") |>
  align(j = 1) |> 
  padding(i = ~is.na(row_title), j = 1, padding.left = 20) |> 
  colformat_md(j = c(9,11, 12), part = "header") |> 
...
m <- list(`Model 1` = lm(charisma ~ leader_gender, data = d), 
          `Model 2` = lm(charisma ~ leader_gender + leader_sleep_condition, data = d)) 
