...
The flextable and gt packages have many, many options and helper functions. The flextable package's `tabulator`, `summarizor`, and `proc_freq` functions are particularly powerful ways of summarizing and describing data. Functions that automate your descriptive statistics tables for you inevitably entail some level of compromise. It is unreasonable for a single function to anticipate the diversity of needs out there. Sometimes you need a table to be a particular way, and you need to do the heaving lifting yourself. After all that, the `apa_style` function will get a flextable or gt table close to APA style. You may need to do some additional styling yourself as well.
...
# gt
gt(d) %>% 
  gt::fmt_number(decimals = 2, columns = is.double) %>%
  gt::tab_spanner_delim(delim = "_") %>% 
  gt::cols_align(align = "center", columns = -1) %>% 
  gt::tab_style(cell_text(style = "italic"), locations = cells_column_labels(-Variable)) %>% 
